...
public class ConsultarSolicitudCmd implements IBaseCmd {
	...
	public void execute(HttpServletRequest req, HttpServletResponse res) throws PersonalsoftException {
		...
		try{
			...
			dto = pedidosCSLFacade.consultarSolicitud(dto);
			if(dto != null){
	...
	private PedidosCSLDTO dtoAssembler(HttpServletRequest req){
		...
		String cedula = req.getParameter("cedula")!=null ? req.getParameter("cedula") : "";
		
		dto.setCodCia(codCia);
		dto.setUsuario(usuario.toUpperCase());
		dto.setNumOrden(numOrden.toUpperCase());
