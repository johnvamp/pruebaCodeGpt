					    ...
					    <tr>
					        ...
					        <td class="bgcajaGris">
					        	<table width="100%" border="0" cellspacing="1" cellpadding="2">
					        		<tr>
					        			<td class="titulosContenido" align="right" width="40%">CRITERIO</td>
					        			<td>
					        				<select id="criterioBuscar" name="criterioBuscar" class="combo04">
					        					<option value="" >-Seleccione-</option>						
												<c:forEach items="${criterios}" var="mot">
													<option value="${mot.id}" ${busqueda.criterio == mot.id ? 'selected':'' }>${mot.nombre}</option>
												</c:forEach>
					        				</select>
					        			</td>
					        		</tr>
					        		...
					        		<tr>
										<td align="center" colspan="2">
											<input type="button" class="boton01" name="btnAceptar" id="btnAceptar" style="cursor: pointer;" onclick="return consultarCasos('${SELECCION_TODO}','','1');" value="CONSULTAR" />
											<input type="button" class="boton01" name="btnExcel" id="btnExcel" style="cursor: pointer;" onclick="return consultarCasos('${SELECCION_TODO}','S','1');" value="GENERAR EXCEL" />
										</td>
									</tr>
					        	</table>
					        </td>
		...
		<c:if test="${!empty casos}">
			<c:if test="${busqueda.totalPaginas > 1}">
				<table width="60%" border="0" cellpadding="0" cellspacing="0" align="center">
					<tr>
						<td align="center">
							<a href="javascript:consultarCasos('${SELECCION_TODO}','','1')" class="linkPaginacion">
								<img src="imagenes/fl_first.png" border="0" title="Primera"/>
							</a>
							<a href="javascript:consultarCasos('${SELECCION_TODO}','','${busqueda.paginaActual - 1}')" class="linkPaginacion">
							...
							<c:if test="${busqueda.paginaActual < busqueda.totalPaginas}">
								...
								<a href="javascript:consultarCasos('${SELECCION_TODO}','','${busqueda.totalPaginas}')" class="linkPaginacion">
						    ...
						    <tr>
						        ...
						        <td class="bgcajaGris">
						        	<table width="100%" border="0" cellspacing="1" cellpadding="2">
										<thead>
							        		<tr>
						        			<th width="5%" class="tituloTablaFijos">Cédula</th>
						        			<th width="10%" class="tituloTablaFijos">Nombre</th>
							       			<th width="5%" class="tituloTablaFijos">Zona</th>
							       			<th width="5%" class="tituloTablaFijos">Campaña Premio</th>
							      			<th width="10%" class="tituloTablaFijos">SKU</th>
							      			<th width="10%" class="tituloTablaFijos">Descripción</th>
							       			<th width="5%" class="tituloTablaFijos">Cantidad</th>
							       			<th width="10%" class="tituloTablaFijos">Motivo</th>
							       			<th width="15%" class="tituloTablaFijos">SKU Sustituto</th>
							       			<th width="5%" class="tituloTablaFijos">Cantidad Entregada</th>
							       			<th width="5%" class="tituloTablaFijos">Tipo Despacho</th>
							    	   		</tr>
										</thead>
										<tbody>		        		
							        		<c:forEach items="${casos}" var="det" varStatus="fila">
...
<%-- 							      				<input type="hidden" id="cedulaTransportista_${fila.count}" name="cedulaTransportista_${fila.count}" value="${det.cedulaTransportista}"/> --%>
							      				...
							      				<input type="hidden" id="dsTransportadora_${fila.count}" name="dsTransportadora_${fila.count}" value="${det.dsTransportadora}"/>
							      				<input type="hidden" id="nombreTransportista_${fila.count}" name="nombreTransportista_${fila.count}" value="${det.nombreTransportista}"/>
							      				<input type="hidden" id="guiaMaster_${fila.count}" name="guiaMaster_${fila.count}" value="${det.guiaMaster}"/>
								        		<tr class="filaTabla11">
								        			...
								        			<td class="${fila.count % 2 == 0 ? 'cel1' : 'cel2'}">${det.motivo}</td>
									       			<td class="${fila.count % 2 == 0 ? 'cel1' : 'cel2'}">${det.skuSustituto}</td>
									       			<td class="${fila.count % 2 == 0 ? 'cel1' : 'cel2'}">${det.cantidadEntregada}</td>
									       			<td class="${fila.count % 2 == 0 ? 'cel1' : 'cel2'}">${det.tipoDespacho}</td>
									       		</tr>
									  		</c:forEach>
										</tbody>		        		
						        	</table>
